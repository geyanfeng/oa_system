...
(function (a) {
	a.jBox = function (b, c) {
		...
		j.push('<a class="jbox-close" title="' + a.jBox.languageDefaults.close + '" onmouseover="$(this).addClass(\'jbox-close-hover\');" onmouseout="$(this).removeClass(\'jbox-close-hover\');" style="position:absolute; display:block; cursor:pointer; top:25px; right:25px; width:15px; height:15px;' + (c.showClose ? '' : 'display:none;') + '"></a>');
		if (g) {
		...
		a.each(b, function (N, O) {
			...
			r.push('<div class="jbox-button-panel text-center" style="' + (P ? '' : 'display:none;') + '">');
			if (!c.isTip) {
				r.push('<span class="jbox-bottom-text" style="float:left;display:block;line-height:25px;"></span>')
			};
			a.each(O.buttons, function (T, U) {
				r.push('<button class="btn m-r-5" value="' + U + '">' + T + '</button>')
			});
			r.push('</div></div>')
		});
