...
<BODY class="fixed-left">
	...
	<DIV id="wrapper">
					...
					<DIV>
						...
						<UL class="nav navbar-nav navbar-right pull-right">
							...
							<LI class="dropdown user-box"><A
								class="dropdown-toggle profile " aria-expanded="true" href="#"
								data-toggle="dropdown"><i class="fa fa-user"
									style="font-size: 24px; line-height: 60px;"></i> </A>
		...
		<DIV class="left side-menu">
			<div class="text-center">
				...
				<h5 class="text-uppercase font-bold m-b-0" id="J_name"
					style="color: white;">
					Hi<span>, ${fns:getUser().name}</span>
				</h5>
			</div>
					...
					<UL>
						...
						<c:forEach items="${fns:getMenuList()}" var="menu"
							...
							<c:if test="${menu.parent.id eq '1'&&menu.isShow eq '1'}">
								<li
									class="has_sub ${not empty firstMenu && firstMenu ? ' active' : ''}">
									<A
									class="waves-effect ${not empty firstMenu && firstMenu ? ' active' : ''}"
									href="${not empty menu.href ? ctx : ''}${not empty menu.href ? menu.href : 'javascript:void(0);'}"
									target="${not empty menu.target ? menu.target : 'mainFrame'}"><I
										class="zmdi zmdi-${not empty menu.icon ? menu.icon : 'view-dashboard'} ti-share"></I><SPAN>${menu.name}</SPAN></A>
									<UL>
										<c:forEach items="${menuList}" var="menu2">
											...
											<c:if
												...
												<li><A data-href=".menu3-${menu2.id}"
													href="${not empty menu2.href ? ctx : ''}${not empty menu2.href ? menu2.href : 'javascript:void(0);'}"
													class="waves-effect"
													target="${not empty menu2.target ? menu2.target : 'mainFrame'}"><SPAN>${menu2.name}</SPAN>
												</A>
													<UL class="menu3">
														<c:forEach items="${menuList}" var="menu3">
															<c:if
																test="${menu3.parent.id eq menu2.id&&menu3.isShow eq '1'}">
																<li><A data-href=".menu3-${menu3.id}"
																	href="${fn:indexOf(menu3.href, '://') eq -1 ? ctx : ''}${not empty menu3.href ? menu3.href : '/404'}"
																	class="waves-effect"
																	target="${not empty menu3.target ? menu3.target : 'mainFrame'}"><SPAN>${menu3.name}</SPAN>
																</A></li>
															</c:if>
														</c:forEach>
													</UL></li>
	...
	<script src="${ctxStatic}/assets/js/iframeResizer.min.js"
