    ...
    <script type="text/javascript">
        ...
        function submitCancelContract(sender){
            var contractId = $(sender).data("contractId"),
                    isCopy = $("#copyFrom").is(':checked')?"true":"false",
                    cancelType = $("#cancelType").val(),
                    cancelReason = $("#cancelReason").val();
            var postData ={isCopy: isCopy, cancelType: cancelType, cancelReason:cancelReason};
            $.ajax({
                        type: 'POST',
        ...
        <table id="contentTable" class="table table-striped table-condensed table-hover">
            ...
            <c:forEach items="${page.list}" var="contract">
                <tr data-json='${fns:toJson(contract)}'>
                    ...
                    <td><c:if test="${contract.cancelFlag eq 1}"><i class="zmdi zmdi-check"></i></c:if> </td>
                    <td>
                    ...
                    <td>
                        ...
                        <td>
                            ...
                            <c:if test="${contract.cancelFlag eq 0}">
                                <shiro:hasPermission name="oa:contract:cancel">
                                    <a href="#" onclick="cancelContract('${contract.id}');">撤销</a>
                                </shiro:hasPermission>
                            </c:if>
                        </td>
...
<div id="modal-cancelContract" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-8">
                                    <select class="form-control" id="cancelType">
                                        <c:forEach items="${fns:getDictList('oa_contract_cancel_type')}" var="cancelType">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-info waves-effect waves-light" id="btnSubmitCancel" onclick="submitCancelContract(this);">确定撤销</button>
